...
export async function viewAccounts() {
    ...
    $('#page').innerHTML = card(
        ...
        `
          ...
          ${table(
              ['name', 'type', 'apr', 'opening_date', 'opening_balance', 'dash', 'archived_at'],
              rows,
              (r) => `
    ...
    const showAccountModal = (a) => {
      ...
      syncInterestUI();
        ...
        modal.querySelector('#am_save').onclick = async () => {
            try {
                const opening_balance_cents = parseCentsFromDollarsString(modal.querySelector('#am_opening_balance').value) ?? 0;

          const is_interest_bearing = Number(modal.querySelector('#am_is_interest').value || 0) ? 1 : 0;
          const aprBps = parseAprBps(modal.querySelector('#am_apr').value);
          if (is_interest_bearing) {
            if (aprBps === null) throw new Error('APR is required when Interest-bearing=Yes');
            if (!Number.isFinite(aprBps)) throw new Error('APR must be a number (e.g., 18.99)');
          }
                ...
                const payload = {
                    ...
                    archived_at: modal.querySelector('#am_archived_at').value || null,

            is_liability: Number(modal.querySelector('#am_is_liability').value || 0) ? 1 : 0,
            is_interest_bearing,
            interest_apr_bps: is_interest_bearing ? Number(aprBps) : null,
            interest_compound: modal.querySelector('#am_compound').value || 'D',
            exclude_from_dashboard: Number(modal.querySelector('#am_exclude_dash').value || 0) ? 1 : 0,
                };

                if (isEdit) await api(`/api/accounts/${a.id}`, { method: 'PUT', body: JSON.stringify(payload) });
